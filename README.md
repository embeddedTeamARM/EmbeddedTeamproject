...
* 프로젝트 소개
    ...
    실생활에서 야간에 동거인이 화장실, 거실, 주방 등의 불을 켜서 잠에서 깨는 경험이 종종 있을 것이다. 빛으로 인해 수면에서 자주 깰 경우에는 사람의 일주기성 리듬이 교란되기 쉬우며, 이로인해 수면의 질이 떨어지고 심할경우에는 비만, 당뇨병 등과 같은 건강문제까지 이어질 수 있다. 우리는 이러한 불편함을 해소하고자 야간에 특정 조명의 밝기를 조절할 수 있는 시스템을 개발하고자 한다.
...
* 프로젝트 개요
    ...
    2. 전체 시스템 구조
        ...
        ![전체 구조-led 설정 관련](https://user-images.githubusercontent.com/81803973/207538973-90469e96-dfba-4512-97b1-0628dae2d450.png)
        
        led 설정부 및 메인 led 전원부는 다음과 같이 위치한다.
        
        a. 메인 led   
            실제 불이 켜지는 등이다.
            
        b. on/off 스위치   
            메인 led의 불을 켜고 끌 수 있도록 하는 버튼이다.
            
        c. 데이터 설정 다이얼   
            로터리 엔코더를 이용하여 다이얼을 돌리면 미리 정한 수치만큼 값을 조절한다.   
            이때, 다이얼을 클릭하면 밝기, 시, 분 순으로 설정이 가능하고, 다이얼을 돌리면 원하는 설정의 값을 조절할 수 있다.
            
        d. led 밝기 인디케이터   
            다이얼로 야간 led의 밝기를 설정할 때, 해당 값이 어느정도의 밝기인지 모를 때가 있다. 이때, 인디케이터를 이용하여 최대 밝기 대비 현재 밝기를 비교할 수 있다.
            
        e. 현재 설정 데이터 인디케이터   
            다이얼로 데이터를 설정할 때 현재 어느 데이터를 수정중인지 확인할 수 있는 기능을 제공한다.
        
        
        ![전체 구조-i2c 및 uart](https://user-images.githubusercontent.com/81803973/207539010-fa3b612e-20a2-4be8-9e70-70e9706fe974.png)
        
        a. 블루투스 모듈   
            스마트폰에서 블루투스 신호를 통해 값을 설정하고, 설정한 값을 다시 스마트폰으로 전송하는 역할을 한다.
            
        b. RTC 모듈   
            현재 시각을 반환한다. 이 RTC 모듈의 시각을 통해 현재 야간인지 야간이 아닌지를 판단할 수 있다.
...
2) 최종 목표  
    ...
    * 4 digit FND는 로터리 엔코더의 입력값이 n초 이상 없다면 꺼지도록 한다.
- - -
...
1) 데이터 설정
    * 1.1 - 블루투스로 설정   
        + 블루투스는 다음과 같은 형태로 데이터를 받는다.   
            ...
            블루투스에서 실시간으로 값이 바뀌는 것이 아니므로, 값을 변경하고자 할 때마다 새로운 데이터를 받아와야 한다. 
